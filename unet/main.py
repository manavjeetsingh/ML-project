...
from unet import UNet, Dilation1, Dilation2,UNet2Layer,UNet2LayerDilation,Residual2LayerUNet
import argparse
...
def train_model(args, model, dataloaders, optimizer, scheduler):
    ...
    #Just to test saving
    with open(args.checkpoint+".mIoU", 'wb') as handle:
        pickle.dump(val_mIoU, handle, protocol=pickle.HIGHEST_PROTOCOL)

    with open(args.checkpoint+".mDice", 'wb') as handle:
        pickle.dump(val_mDice, handle, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    for epoch in range(args.epochs):
        ...
        for phase in ['train', 'val']:
            ...
            val_mIoU[epoch+1]=metrics['mIoU']
            val_mDice[epoch+1]=metrics['mDice']
...
def main(args):
    ...
    num_class = 1
    if args.skip == 'concat':
        model = UNet(num_class, load_pretrained_encoder_layers=args.load_pretrained).to(device)
    # elif args.skip == 'sum':
    #     model = SumUNet(num_class, args.load_pretrained).to(device)
    elif args.skip == 'dilation1':
        model = Dilation1(num_class, load_pretrained_encoder_layers=args.load_pretrained).to(device)
    elif args.skip == 'dilation2':
        model = Dilation2(num_class, load_pretrained_encoder_layers=args.load_pretrained).to(device)
    elif args.skip == '2layer':
        model = UNet2Layer(num_class, load_pretrained_encoder_layers=args.load_pretrained).to(device)
    elif args.skip == '2layerdilation':
        model = UNet2LayerDilation(num_class, load_pretrained_encoder_layers=args.load_pretrained).to(device)
    elif args.skip == 'residual2layer':
        model = Residual2LayerUNet(num_class, load_pretrained_encoder_layers=args.load_pretrained).to(device)
    else:
        exit(1)
...
def evaluate(args):
    ...
    for mtype in ['float32','float16','int8']:
    # for mtype in ['float16']:
    # for mtype in ['float32','int8']:
        st=time.time()
        half=False
        if args.skip == 'concat':
            model = UNet(num_class, args.load_pretrained).to(device)
        ...
        try:
            ...
            # print(args.skip,args.checkpoint)
            checkpoint=f'{args.skip}_checkpoint.pth'
            model.load_state_dict(torch.load(checkpoint))
        lines="-------------------"
        print(lines,mtype,lines)
        if mtype=='float32':
            print("Doing nothing")
        elif mtype=='float16':
            print("Halfing the model")
            # half=True
            # model=model.half()
            model= torch.ao.quantization.quantize_dynamic(model, 
                {
                    torch.nn.Conv2d,
                    torch.nn.ReLU,
        ...
        mIoU, mDice = evaluate_segmentation(model, data_loader_val, save_images=True,half=half)
...
def evaluate_segmentation(model, data_loader, save_images=False,half=False):
    ious = []
    dices = []

    for samples, targets in data_loader:
        if half:
            inp = samples.tensors.type(torch.cuda.HalfTensor)
        else:
            inp = samples.tensors
...
if __name__ == '__main__':
    ...
    parser.add_argument('--skip', type=str, choices=['concat', 'dilation1','dilation2','2layer','2layerdilation','residual2layer'], default='concat',
                        help='Skip connection strategy: either concatenation (concat) or summation (sum).')
    parser.add_argument('--checkpoint', type=str, default='checkpoint.pth',
                        help='Path to checkpoint file.')

    args = parser.parse_args()
    args.checkpoint = f'{args.skip}_{args.load_pretrained}_{args.checkpoint}'
    print(args)
    if not args.eval:
        main(args)
    else:
